import os
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from src.graph import definir_grafo
from langchain_google_vertexai import ChatVertexAI

app = FastAPI()

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

PROJECT_ID = os.getenv("GCP_PROJECT_ID", "iagen-gcp-cwmi")
CORPUS_ID = "projects/iagen-gcp-cwmi/locations/us-west1/ragCorpora/4532873024948404224"

llm = ChatVertexAI(model="gemini-2.0-flash-lite", project=PROJECT_ID)
grafo_donchevas = definir_grafo(llm, CORPUS_ID)

# Memoria de persistencia para el dominio
estado_sesion = {"dominio": "PROFESIONAL"}

@app.post("/chat")
async def chat(request: dict):
    try:
        inputs = {
            "consulta": request["prompt"], 
            "dominio": estado_sesion["dominio"]
        }
        
        resultado = grafo_donchevas.invoke(inputs)
        
        # Actualizamos la memoria para la siguiente pregunta corta
        estado_sesion["dominio"] = resultado["dominio"]
        
        return {
            "respuesta": resultado["respuesta"],
            "dominio": resultado["dominio"]
        }
    except Exception as e:
        return {"respuesta": f"Error: {str(e)}"}

if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("PORT", 8080))
    uvicorn.run(app, host="0.0.0.0", port=port)